...
await step("2. narrate (ElevenLabs + timestamps)", () => run("npx", ["tsx", "src/scripts/narrate-timed.ts"]));
await step("3. captions + props", () => run("npx", ["tsx", "src/scripts/prepare-render.ts"]));
await step("4. images (Cloudflare FLUX 16:9)", () => run("npx", ["tsx", "src/scripts/gen-scenes-cf.ts"]));
...
await step("7. render (Remotion)", () =>
  run("npx", ["remotion", "render", "src/remotion/index.ts", "StoryVideo", "out/story.mp4", "--props=./out/props.json", "--concurrency=4", "--log=error"]));
