...
// each beat's clip plays exactly while its narration is spoken; the last clip holds the fade tail
...
const props = {
  ...
  lines, title: story.onScreenTitle || story.title, channel: "InfotainmentStu",
...
console.log(`beats=${beats.length} words=${words.length} dur=${narrationDur.toFixed(1)}s frames=${durationInFrames} clips=${timeline.length}`);
